"""04_evaluate_model.py — full evaluation (model + baselines) then generate plots.

Runs evaluate() with all methods ["model", "zero", "linear", "cubic_spline",
"gp_matern32"], writes results.pkl into --out-dir, then calls make_all_plots()
to produce figures/ under the same directory.

Usage
-----
    python scripts/04_evaluate_model.py \\
        --checkpoint outputs/run1/best.pt \\
        --out-dir outputs/run1/eval

    python -m lisa_gap_imputer.scripts.04_evaluate_model \\
        --checkpoint outputs/run1/best.pt \\
        --out-dir outputs/run1/eval
...
_ALL_METHODS = ["model", "zero", "linear", "cubic_spline", "gp_matern32"]
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="04_evaluate_model",
        description=(
            "Run full evaluation (trained model + all baselines) and generate "
            "diagnostic plots. Writes results.pkl and figures/ into --out-dir."
        ),
    )
    p.add_argument(
        "--checkpoint",
    ...
    p.add_argument(
        ...
        required=True,
        help=(
            "Output directory. Receives results.pkl and a figures/ sub-directory "
            "containing PNG, PDF, and CSV files."
        ),
    )
    p.add_argument(
        "--n-test",
...
def main(argv: list[str] | None = None) -> int:
    ...
    from lisa_gap_imputer.plotting import make_all_plots

    checkpoint = Path(args.checkpoint)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    results_pkl = out_dir / "results.pkl"
    ...
    print(
        f"[04_evaluate_model] Running methods: {_ALL_METHODS}\n"
    ...
    print(f"[04_evaluate_model] Results written to {result_path}. Generating plots ...")

    plot_paths = make_all_plots(result_path, figures_dir)

    print(
        f"[04_evaluate_model] Done. {len(plot_paths)} figure(s) written to {figures_dir}"
    )
    return 0
